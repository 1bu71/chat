...
void ChatService::login(const TcpConnectionPtr &conn, const json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPassword() == pwd)
    {
        ...
        {
            // 聊天服务，需要服务器主动向客户端推送消息，所以需要维护一个map表，存储用户的连接
            {
...
void ChatService::loginout(const TcpConnectionPtr &conn, const json &js, Timestamp time)
{
    ...
    _redis.unsubscribe(userid); // 取消订阅
    //更新用户的信息
    User user;
    user.setId(userid);
    _userModel.updateState(user);
...
void ChatService::clientCloseException(const TcpConnectionPtr &conn)
{
    ...
    _redis.unsubscribe(user.getId()); // 取消订阅
    if (user.getId() != -1)
    {
...
void ChatService::groupchat(const TcpConnectionPtr &conn, const json &js, Timestamp time)
{
    int userid = js["id"].get<int>();
    int groupid = js["groupid"].get<int>();
    vector<int> groupUsers = _groupModel.queryGroupsUsers(userid, groupid);
...
void ChatService::handleRedisSubscribeMessage(int userid, string msg)
{
    ...
    _offlineMsgModel.insert(userid, msg); // 用户离线，存储离线消息
}